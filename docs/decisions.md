...
- **A pasted Gatherer link gets the same fallback** (v2.17.4). `_fetch_gatherer` used to raise whenever it could not produce a multiverse id, so the single-link path stayed broken after the import path was fixed. It now serves the Scryfall image instead, and only fails when neither site recognises the card.
- **Gatherer's set abbreviations are its own - do not feed them to Scryfall.** A new-style link is `gatherer.wizards.com/<SET>/<locale>/<number>/<name-slug>`, and `<SET>` is Gatherer's code: Urza's Saga is `UZ` there and `usg` on Scryfall, so `/cards/uz/291` 404s for a card both sites have. The **name slug** is the part they agree on, so it is parsed out of the link and used as the fallback lookup (fuzzy, because the slug has already dropped apostrophes: `tyvars-stand` → Tyvar's Stand), then narrowed to the printing whose collector number matches. Do not try to maintain a set-code mapping table; the name is free and stays correct.
- **A Scryfall gallery pick is queued by card id, not by its image url** (v2.17.10). The gallery's `download` points at the **front face**, so a double-faced card picked there arrived as half a card. The pick also carries the Scryfall `identifier`, and `https://api.scryfall.com/cards/<id>` routes through `_card_from_reference` case 2, so `scryfall.fetch` resolves every face. Safe against the global preferences, and checked rather than assumed: an api url makes `ref_names_a_printing` true, so **best_scan** stands down, and `_ref_pins_language` true, so the **language picker** does too. An explicit pick beats both, which is the rule already set for pasted links. Used by the queue and by *Change art* in the export dialog.
- **This was the fourth route to lose a back.** Order XML (v2.17.6), MPC gallery (v2.17.7), the race that broke the MPC fix (v2.17.8), and the Scryfall gallery (v2.17.10). Any new way of getting a card into the queue needs checking against a double-faced card before it ships; the failure is silent, because the sheet quietly falls back on back.png.
- **A double-faced card picked from the MPC gallery pulls its own back** (v2.17.7). A gallery pick is one image, so before this a DFC chosen by art fell through to the shared back.png; the order-XML fix in v2.17.6 did not touch this path. The back is not in the search result, but it *is* in the catalogue under its own card name, so: Scryfall says what the second face is called (`back_face_name`), the catalogue is searched for that name, and the winner is the entry by the **front's own contributor**, preferring the one whose art variant suffix matches. Contributors upload both faces together, so this is a lookup rather than a guess: verified against Rona, Herald of Invasion, where Chilli_Axe, WillieTanner (a) and (b), and PsilosX each pair correctly. When only another contributor has the back it is still used, and the queue row says so.
- **What counts as two faces is `_png_urls`'s test, reused.** Split, flip and adventure cards also carry two `card_faces`, but Scryfall gives those a top-level image and no per-face images. `back_face_name` checks the same thing the downloader does, so the two can never disagree about how many files a card is worth. Verified: Fire // Ice and Bonecrusher Giant return None.
- The lookup runs **off the UI thread** and fails soft. It costs a Scryfall call plus a catalogue search per MPC pick, and nothing is downloaded until Upscale all, so an unreachable Scryfall must never stop a card from queueing.
- **An MPC order's `<backs>` section is the double-faced cards** (v2.17.6). It was documented in the module comment but never parsed, so every imported MDFC silently fell through to the shared back.png. Its `<card>` entries are keyed by the **same slot numbers the fronts use**, so slot 1 in `<backs>` belongs to whichever front holds slot 1. Verified against MPC Autofill's own `ExportXML` tests, not guessed. A front entry covering several slots is **split** when those slots do not share a back, since letting the first slot speak for all of them would hand copies a back they were never assigned.
- **`<cardback>` is deliberately NOT imported.** It is the shared back for every ordinary slot, which is what the app's own card-back setting already does; importing it would silently override a user's chosen back.
- **Both faces download with `-front` / `-back` suffixes**, which is the convention `ExportDialog._pairs()` already uses for Scryfall DFCs. `upscale(rename=False)` keeps the stem, so the suffix survives into the output filename and nothing downstream needs to know the card came from MPC.
...
- **A download basename must be unique or cards vanish silently.** `download_to_temp` writes `TEMP_FOLDER/<basename>.png`, so two queue entries sharing a basename overwrite each other and the user just gets fewer cards. An MPC order names every entry by card alone, so a real 89-card order with 2 Plains, 3 Islands and 2 Arcane Signets collapsed to **85 files**. The order's `slot` is unique per entry and is now folded into the name; gallery picks fold in the catalogue's `identifier` for the same reason (two Yu-Gi-Oh artworks from one set, or two MPC arts by one contributor, collided the same way).
