...
- **Marks collide with cards** — this is unavoidable, not a bug: on A4 the 3×3 block starts 10.5 mm from the edge while the marks span 15.9→35.9 mm. So `_reg_blocked_slots` computes which slots a mark's clear box overlaps and those slots are left EMPTY; cards flow into the rest (9 → 6 per sheet, or 5 with 4 marks). `build_pdf` and the preview share this mapping (`usable[k]`), so what you see is what prints.
