...
- **The mark inset defaults to Silhouette Studio's own 15.875 mm** (`REG_STUDIO_INSET_MM`), changed from 10 in v2.13.0. A template built in Studio is laid out around Studio's value, so marks exported at 10 do not land on it — that was the reported ProxySheets mismatch.
- That default **costs card slots** on some page/layout pairs, because a deeper inset pushes the marks further onto the grid. Measured: A4 3×3 goes 9 usable → 6 (5 with 4-mark), Letter 4×2 goes 8 → 5 (4), Letter 7-card goes 7 → 6 (5). A4 4×2 and A4 7-card are unaffected, and Letter 3×3 already blocked 3-4 slots at either value. The minimum is still 10, so anyone who prefers density can set it back.
- Existing users are not moved: `entry_row` reads the saved `reg_inset` before the default, so only fresh installs pick up 15.875.
