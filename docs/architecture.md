...
- `upscale.py` - pipeline: normalize → (stand a sideways card up) → (trim MPC bleed) → Real-ESRGAN x4 → fit 2976×4160 → set 1200 DPI. No GPU: LANCZOS resize. `_orient_to_card` turns an image that is the card's shape lying down, which is what a Riftbound Battlefield is; anything else wide is left alone.
...
- `riftbound.py` - Riftbound catalogue (api.riftcodex.com), same interface again. **No API key**, which is what still blocks apitcg.com and with it One Piece / Digimon / Dragon Ball. Images are Riot's own CDN at 744×1039. `search` merges an uncapped `exact` name lookup with a `fuzzy` one the server caps at 10.
...
- `wiki/` - source of the GitHub wiki, one `.md` per page (`Home.md`, `_Sidebar.md`, `_Footer.md` included). Kept here so the guides are versioned and reviewed like the code, the same reasoning that keeps `site/` on main. The wiki repo itself (`cardwright.wiki.git`) is a push target, not a place to edit: its footer says so. After changing anything in `wiki/`, run `python tools/sync_wiki.py` - it mirrors, so a page deleted here leaves the wiki too, and a rerun with nothing changed pushes nothing.
